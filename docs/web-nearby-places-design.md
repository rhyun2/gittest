...
```
data/seeds/*.txt  ──(빌드 타임, 키 사용)──→  web/data/places.json  ──→  브라우저(키 불필요)
```

런타임 데이터 소스 선택은 한 줄로 요약된다.

```
큐레이션에 반경 내 결과가 있으면 → 그것을 쓴다 (사진·평점·한줄평 포함)
없으면                          → 카카오 실시간 검색 (기존 경로)
```

이 순서 덕분에 부수 효과가 하나 생긴다. 큐레이션이 그 지역을 덮고 있으면 **카카오 앱키 없이도 앱이 동작한다.**

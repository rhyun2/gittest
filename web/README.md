...
**좌표가 없으면 거리를 잴 수 없어 그 장소는 목록에서 빠진다.** 두 가지 방법이 있다.

- **TourAPI 키를 쓴다** — 좌표·사진·요약이 한 번에 채워진다. [공공데이터포털](https://www.data.go.kr/data/15101578/openapi.do)에서
  "한국관광공사 국문 관광정보 서비스" 활용신청 → 마이페이지에서 일반 인증키(Decoding) 복사
- **직접 적는다** — 카카오맵에서 장소를 우클릭 → "좌표 복사" 하면 위도, 경도 순서로 나온다

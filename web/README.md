...
**같은 와이파이에서 `http://192.168.x.x:8000`으로 접속하는 방법은 동작하지 않는다.** iOS Safari는 HTTPS가 아닌 출처에서 Geolocation을 차단하고, `localhost` 예외는 그 기기 자신에게만 적용되기 때문이다. 이 경우 앱은 🔒 "HTTPS에서만 동작해요" 화면을 띄운다.

두 가지 방법이 있다.

**A. GitHub Pages 배포** — 아래 [배포](#배포-github-pages) 참고. 워크플로가 이미 준비돼 있다.
...
| 🔒 "HTTPS에서만 동작해요" | `localhost`가 아닌 주소로 접속했다. 6단계 참고 |

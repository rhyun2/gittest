...
## 장소 직접 정리하기 (사진·평점·한줄평)

카카오 실시간 검색은 사진도 평점도 주지 않는다. 그래서 **직접 정리한 장소 목록**을 따로 둔다.
그 지역 데이터가 있으면 사진이 붙은 큐레이션 목록이 뜨고, 없으면 기존 카카오 검색으로 넘어간다.

```
data/seeds/jeju.txt          ← 손으로 관리하는 장소 목록
        │  python3 scripts/build_places.py
        ↓
web/data/places.json         ← 생성물. 커밋한다
        ↓
앱이 읽어서 거리순으로 표시
```

수집이 **빌드 시점**에 끝나므로 브라우저는 완성된 JSON만 읽는다. TourAPI를 런타임에 부를 때 걸리는
CORS 문제도, 서비스키가 노출되는 문제도 생기지 않는다.

### 1. 씨드 파일 편집

`data/seeds/jeju.txt` 를 연다. 형식은 `장소명 | 위도,경도 | 평점 | 한줄평` 이고 **장소명만 필수**다.

```
[관광지]
성산일출봉 | 33.4580,126.9427 | 4.6 | 새벽에 오르면 사람이 적다
비자림     |                  |     |
```

다른 지역을 추가하려면 `data/seeds/` 안에 `.txt` 파일을 더 만들면 된다. 전부 읽어 합친다.

### 2. 좌표 채우기

**좌표가 없으면 거리를 잴 수 없어 그 장소는 목록에서 빠진다.** 장소명만 적어 두었다면
아래 스크립트가 카카오 검색으로 좌표를 찾아 씨드 파일에 직접 채워 넣는다.

```bash
# 먼저 어떻게 매칭되는지만 확인 (파일은 안 건드림)
KAKAO_REST_API_KEY=키값 python3 scripts/geocode_seeds.py --dry-run

# 결과가 괜찮으면 실제로 채우기
KAKAO_REST_API_KEY=키값 python3 scripts/geocode_seeds.py
```

> **REST API 키**를 쓴다. 웹앱이 쓰는 JavaScript 키와 **다른 키**이고, 도메인 제한이
> 걸리지 않으므로 진짜 비밀이다. 환경변수로만 넘기고 절대 커밋하지 않는다.
> 발급: 카카오 개발자 콘솔 → 내 애플리케이션 → 앱 키 → **REST API 키**
> (JavaScript 키를 넣으면 401이 뜬다)

씨드 파일 상단의 지시자로 검색 정확도를 조절한다.

```
#! region: 제주                          검색어 앞에 붙어 타 지역 동명 가게를 걸러낸다
#! bbox: 33.10,126.10,33.60,127.00      이 범위 밖 결과는 버리고 다음 후보를 본다
```

이 스크립트는 좌표와 함께 **카카오맵 상세 페이지 주소**도 모아 `data/kakao-links.json` 에 저장한다.
씨드는 손으로 고치는 파일이라 긴 URL로 지저분해지지 않게 따로 뒀다. 이 링크가 있으면 앱에서
장소를 눌렀을 때 사진·리뷰·영업시간이 있는 상세 페이지로 가고, 없으면 좌표로 지도만 연다.

좌표와 링크가 **둘 다 있는 줄만** 건너뛴다. 좌표만 채워 둔 기존 씨드가 있다면 그냥 다시 돌리면
링크가 뒤늦게 붙는다(줄 내용은 건드리지 않는다). 전부 다시 찾으려면 `--overwrite` 를 붙인다.
매칭된 장소명이 씨드의 이름과 다르면 `← '실제 이름' 로 매칭` 으로 표시되니 눈으로 확인하면 된다.

**대안**: 손으로 적어도 된다. 카카오맵에서 장소를 우클릭 → "좌표 복사" 하면 위도, 경도 순서로 나온다.
TourAPI 키가 있다면 3단계에서 좌표·사진·요약이 함께 채워지지만, 관광지 위주라 음식점·카페는 잘 안 잡힌다.

### 3. 생성

```bash
python3 scripts/build_places.py                     # 씨드에 적힌 값만으로
TOUR_API_KEY=발급받은키 python3 scripts/build_places.py   # 사진·요약까지 채워서
```

몇 곳이 만들어졌는지, 사진이 몇 곳에 붙었는지, 좌표가 없어 빠진 장소가 무엇인지 알려준다.
결과인 `web/data/places.json` 을 커밋하면 배포된다.

평점과 한줄평은 자동으로 수집하지 않는다. 포털 평점을 긁어오는 것은 약관 위반 소지가 있어,
직접 다녀와서 매긴 점수를 씨드에 적는 방식이다. 비워두면 화면에 안 나온다.
